...
void unicycle_dyn_sim::Prepare(void)
{
    ...
    FullParamName = ros::this_node::getName()+"/I";
    if (false == Handle.getParam(FullParamName, I))
        ROS_ERROR("Node %s: unable to retrieve parameter %s.", ros::this_node::getName().c_str(), FullParamName.c_str());
    ...
    /* ROS topics */
    vehicleCommand_subscriber = Handle.subscribe("/robot_input", 1, &unicycle_dyn_sim::vehicleCommand_MessageCallback, this);
    ...
    simulator = new unicycle_dyn_ode(dt);

    /* Initialize simulator class */
    simulator->setInitialState(x0, y0, theta0, v0, omega0);
    simulator->setVehicleParams(m, I);

    ROS_INFO("Node %s ready to run.", ros::this_node::getName().c_str());
}

void unicycle_dyn_sim::RunPeriodically(void)
{
...
void unicycle_dyn_sim::Shutdown(void)
{
    // Delete ode object
    delete simulator;
...
void unicycle_dyn_sim::vehicleCommand_MessageCallback(const std_msgs::Float64MultiArray::ConstPtr& msg)
{
    // Input command: t, msg->data[0]; longitudinal force, msg->data[1]; steering torque, msg->data[2]
    /*  Set vehicle commands */
    simulator->setReferenceCommands(msg->data.at(1), msg->data.at(2));
}

void unicycle_dyn_sim::PeriodicTask(void)
{
    /*  Integrate the model */
    simulator->integrate();
    ...
    double v, omega;
    simulator->getVelocity(v, omega);

    double force, torque;
    simulator->getCommands(force, torque);
    ...
    vehicleStateMsg.data.push_back(omega);
    vehicleStateMsg.data.push_back(force);
    vehicleStateMsg.data.push_back(torque);
